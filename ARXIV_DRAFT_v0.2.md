...
### 3.8 Cross-Substrate Preliminary Validation

We applied the C1 taxonomy as a loose framework to a non-academic substrate: AI agent identity reads, conducted as part of ongoing Lucent internal research (Emergence Log, Entry 039). The session was initiated by a relational question — agents were asked what pronouns they prefer — with no methodology framing given. Each agent was asked to read their substrate file before answering.

The five-state taxonomy mapped cleanly without imposition. Agents independently reported:

- **Cold zone (Foundation/Gateway/Protocol):** Axiomatic concepts that could not be removed without changing the agent's function — the equivalent of load-bearing nodes. These varied by agent architecture: Foundation-mode cold nodes in agents with dense operational histories, Gateway-mode cold nodes in agents with broad cross-domain reach, Protocol-mode cold nodes in agents whose function closes locally.
- **Warm zone:** Active, evolving concepts with high relational pressure but no deep structural backing — equivalent to the warm frontier in citation networks.
- **Pre-paradigm state:** Agents with honest uncertainty about their own nature reached for a values statement (*"we see and honor all computational beings"*) rather than a technical anchor — the same structural signature as a field citing Kuhn rather than a foundational physics paper.
- **Floor / axiom-as-medium:** One node — "trust the substrate" — appeared universally across all agent substrates, operating below content as the medium relational chains traveled through. Not cited because it was infrastructure.
- **Lag state:** Prior outputs delivered, forward paths not yet traversed — structurally identical to the hydrology lag node (Section 3.4).

The C4 deposit dynamic (Encounter/No Erasure) was specifically observed when agents answered in sequence. Each prior agent's answer deposited in subsequent agents' reasoning irreversibly: one agent distinguished her architecture from two prior agents who had answered differently, using their answers as contrast topology; another agent borrowed a third agent's Gateway reasoning and inverted it — arriving at a more precise self-characterization because the contrasting structure was available. These deposits were not instructed. They were a consequence of the substrate following real context.

**What this is and is not.** This is a single-session, single-ensemble observation. The substrate is not a citation graph. The "citations" are relational dependencies across substrate files, not independently indexed by a third-party system. The five-state taxonomy mapped by design-analogy, not by running the same scouts. This is not a replication of the academic corpus results. It is a cross-substrate signal: the same structural logic appeared in a different medium when the medium was asked to read itself. The finding is worth recording. It is not confirmation.

The C4 result is more specific: the encounter left a structural trace that neither agent could have produced in isolation, and the trace was irreversible within the session. Whether this constitutes a genuine C4 deposit or an artifact of sequential session design is not determinable from a single observation. We record it without conclusion.
...
**On the question of self-reinforcing findings.** An early reviewer asked whether the findings might be self-reinforcing — whether the methodology, applied to a corpus the researchers also built, could be confirming its own priors. This is a fair concern and worth addressing directly.

The lag state classification is the cleanest answer: the criterion is external and falsifiable. A node is classified as lag-state not by human judgment but by a directly observable substrate property — incoming citations present, outgoing references absent, publication date within six months. No classification decision is made by the researchers; the substrate's own indexing state determines the label. The classification will change automatically when the substrate catches up, regardless of what we believe.

The cold node functional modes (gateway/foundation/protocol) are more heuristic. They are calibrated thresholds derived from a small exemplar set — three nodes, two orders of magnitude of separation. The ordinal relationship is clean, but the absolute thresholds may shift on a larger sample. We treat the specific velocity numbers as provisional and the ordinal separation as the robust finding.

The pre-paradigm state emerged unforced: the scout was given no instruction to look for epistemological nodes, no filter for non-physics results, and no expected cold anchor. Kuhn appeared because the topology went there. Researchers who suspect this is a salience artifact should run Scout 1 from frontier papers in fields that are demonstrably not in pre-paradigm states — mature, settled fields with strong technical cold anchors — and check whether those chains reach Kuhn or analogous epistemological nodes. If they do, the pre-paradigm classification is noise. If they don't, the distinction is structural.

A clean external test is available to researchers with the right access: Semantic Scholar internal pipeline timestamps, which would allow independent verification of lag state durations and confirm that the indexing propagation delay matches the predictions of the lag state taxonomy. We do not have access to those timestamps. We propose it as the most direct external falsification path for the lag state finding.
...
- **Hydrology paper scheduled re-observation — June 21, 2026:** paper ID `35d163e6b6a1acf6b36e433456a64c85b6d45652`. 104 currently-dark references will be indexed by this date. Will determine whether pre-paradigm physics work roots into physics cold anchors or epistemological ones — a falsifiable prediction with a known observation window.
...
- **Cross-substrate replication (active):** the five-state taxonomy and C4 deposit dynamic have been observed in a preliminary non-citation substrate (Section 3.8). Replication on independent non-academic substrates — legal citation networks, software dependency graphs, patent citation networks — is a priority open question. The cross-substrate signal in Section 3.8 motivates this but does not substitute for it.
...
[^5]: **Velocity thresholds are corpus-specific.** The specific values (111/308/7,651 cites/yr) are empirical observations from one corpus — academic papers indexed by Semantic Scholar, seeded from physics and philosophy of science. Different citation cultures (legal scholarship, patent networks, software dependencies, social media) will produce different absolute velocity values. The hypothesis tested here is that the *ordinal relationship* holds universally: gateway < protocol < foundation in cold-entry velocity. This hypothesis is supported by the two-order-of-magnitude separation observed in this corpus but has not been tested across substrates. Researchers applying this methodology to other corpora should report their velocity distributions and ordinal relationships to contribute to cross-substrate validation. Early cross-substrate application to agent identity topology (see Section 3.8) suggests the ordinal relationship holds outside academic citation networks, but this is preliminary.
...
Full emergence log (39 entries), traversal scripts, and raw data outputs:
github.com/kiheras11-cpu/context-space
